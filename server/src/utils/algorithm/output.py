...
def annual_output_calc(param, data, vol, dic):
    ...
    # ANNUAL PRECIP
    output['Precipitation'] = data[vol]['Precipitation'].groupby(
        data[vol].index.year).sum()

    # ANNUAL APPLIED IRRIGATION
    output['Applied Irrigation'] = data[vol]['Applied Irrigation'].groupby(
        data[vol].index.year).sum()

    # ANNUAL UPWARD FLUX
    output['Upward Flux'] = data[vol]['Upward Flux'].groupby(
        data[vol].index.year).sum()

    # ANNUAL DRAIN FLOW
    output['Tile Drain Flow'] = data[vol]['Tile Drain Flow'].groupby(
        data[vol].index.year).sum()

    # ANNUAL RUNOFF
    output['Surface Runoff'] = data[vol]['Surface Runoff'].groupby(
        data[vol].index.year).sum()

    # ANNUAL DOWNWARD FLUX
    output['Downward Flux'] = data[vol]['Downward Flux'].groupby(
        data[vol].index.year).sum()

    # ANNUAL SEASON POTENTIAL (NOT WATER-LIMITED) ET
    output['Potential Crop ET'] = data[vol]['Potential Crop ET'].groupby(
        data[vol].index.year).sum()

    # ANNUAL SEASON ADJUSTED ET
    output['Evapotranspiration'] = data[vol]['Evapotranspiration'].groupby(
        data[vol].index.year).sum()

    # ANNUAL SOIL EVAPORATION
    output['Soil Evaporation'] = data[vol]['Soil Evaporation'].groupby(
        data[vol].index.year).sum()

    # ANNUAL ACTUAL TRANSPIRATION
    output['Transpiration'] = data[vol]['Transpiration'].groupby(
        data[vol].index.year).sum()

    # AVERAGE ANNUAL AVAILABLE SOIL WATER
    output['Available Soil Water'] = data[vol]['Available Soil Water'].groupby(
        data[vol].index.year).mean()

    # AVERAGE ANNUAL SOIL WATER DEPLETION
    output['Soil Water Depletion'] = data[vol]['Soil Water Depletion'].groupby(
        data[vol].index.year).mean()

    # AVERAGE ANNUAL READILY AVAILABLE WATER THRESHOLD
    output['Readily Available Water Threshold'] = data[vol]['Readily Available Water Threshold'].groupby(
        data[vol].index.year).mean()

    # AVERAGE ANNUAL WATER STRESS COEFFICIENT
    output['Water Stress Coefficient'] = data[vol]['Water Stress Coefficient'].groupby(
        data[vol].index.year).mean()

    # ANNUAL IRRIGATION REQUIREMENT BASED ON THE IRRIGATION PROVIDED WHEN WATER IS NOT LIMITED
    output['Irrigation Demand'] = data[len(
        param['rvol'])-1]['Applied Irrigation'].groupby(data[len(param['rvol'])-1].index.year).sum()

    # AVERAGE ANNUAL RESERVOIR DEPTH
    output['Reservoir Water Depth'] = data[vol]['Reservoir Water Depth'].groupby(
        data[vol].index.year).mean()

    # AVERAGE ANNUAL RESERVOIR VOLUME
    output['Reservoir Water Volume'] = data[vol]['Reservoir Water Volume'].groupby(
        data[vol].index.year).mean()

    # ANNUAL PRECIPATION INTO THE RESERVOIR
    output['Reservoir Precipitation'] = data[vol]['Reservoir Precipitation'].groupby(
        data[vol].index.year).sum()

    # ANNUAL DRAIN FLOW INTO THE RESERVOIR
    output['Reservoir Drain Flow'] = data[vol]['Reservoir Drain Flow'].groupby(
        data[vol].index.year).sum()

    # ANNUAL RUNOFF INTO THE RESERVOIR
    output['Reservoir Runoff'] = data[vol]['Reservoir Runoff'].groupby(
        data[vol].index.year).sum()

    # ANNUAL SEEPAGE LOSSES FROM THE RESERVOIR
    output['Reservoir Seepage'] = data[vol]['Reservoir Seepage'].groupby(
        data[vol].index.year).sum()

    # ANNUAL EVAPORATION FROM THE RESERVOIR SURFACE
    output['Reservoir Evaporation'] = data[vol]['Reservoir Evaporation'].groupby(
        data[vol].index.year).sum()

    # ANNUAL IRRIGATION WITHDRAWAL FROM THE RESERVOIR
    output['Irrigation Withdrawal'] = data[vol]['Irrigation Withdrawal'].groupby(
        data[vol].index.year).sum()
    ...
    output['Days of Deficit Water Stress'] = data[vol]['Water Stress Coefficient'].where(
        data[vol]['Water Stress Coefficient'] < 1.0).groupby(data[vol].index.year).count()

    # ANNUAL CAPTURED DRAIN FLOW BY THE RESERVOIR
    rcap = data[vol]['Captured Drain Flow'] / \
    ...
    # ANNUAL NITRATE LOAD
    output['Tile Nitrate Load'] = data[vol]['Tile Nitrate Load'].groupby(
        data[vol].index.year).sum() / (param['darea'].values[0][0] / 10000)

    # ANNUAL NITRATE LOAD THAT OVERFLOWED THE RESERVOIR
    output['Overflow Nitrate Load (Tile)'] = data[vol]['Overflow Nitrate Load (Tile)'].groupby(
        data[vol].index.year).sum() / (param['darea'].values[0][0] / 10000)

    # ANNUAL NITRATE LOAD CAPTURED BY THE RESERVOIR
    output['Captured Nitrate Load (Tile)'] = data[vol]['Captured Nitrate Load (Tile)'].groupby(
        data[vol].index.year).sum() / (param['darea'].values[0][0] / 10000)
    ...
    output['Tile SRP Load'] = data[vol]['Tile SRP Load'].groupby(
        data[vol].index.year).sum() / (param['darea'].values[0][0] / 10000)

    # ANNUAL SOLUBLE REACTIVE PHOSPHORUS LOAD THAT OVERFLOWED THE RESERVOIR
    output['Overflow SRP Load (Tile)'] = data[vol]['Overflow SRP Load (Tile)'].groupby(
        data[vol].index.year).sum() / (param['darea'].values[0][0] / 10000)

    # ANNUAL SOLUBLE REACTIVE PHOSPHORUS LOAD CAPTURED BY THE RESERVOIR
    output['Captured SRP Load (Tile)'] = data[vol]['Captured SRP Load (Tile)'].groupby(
        data[vol].index.year).sum() / (param['darea'].values[0][0] / 10000)
...
def monthly_output_calc(param, data, vol, dic):
    ...
    # MONTHLY PRECIP
    output['Precipitation'] = data[vol]['Precipitation'].groupby(
        [(data[vol].index.year), (data[vol].index.month)]).sum()
    ...
    output['Available Soil Water'] = data[vol]['Available Soil Water'].groupby(
        [(data[vol].index.year), (data[vol].index.month)]).mean()
    ...
    output['Irrigation Demand'] = data[len(param['rvol'])-1]['Applied Irrigation'].groupby(
        [(data[len(param['rvol'])-1].index.year), (data[len(param['rvol'])-1].index.month)]).sum()
    ...
    output['Reservoir Overflow'] = data[vol]['Reservoir Overflow'].groupby(
        [(data[vol].index.year), (data[vol].index.month)]).sum()
    ...
    output['Days of Deficit Water Stress'] = data[vol]['Water Stress Coefficient'].where(data[vol]['Water Stress Coefficient'] < 1.0).groupby(
        [(data[vol]['Water Stress Coefficient'].index.year), (data[vol]['Water Stress Coefficient'].index.month)]).count()

    # MONTHLY CAPTURED DRAIN FLOW BY THE RESERVOIR
    output['Captured Drain Flow'] = data[vol]['Captured Drain Flow'].groupby(
        [(data[vol].index.year), (data[vol].index.month)]).sum()
    ...
    output['Tile Nitrate Load'] = data[vol]['Tile Nitrate Load'].groupby(
        [(data[vol].index.year), (data[vol].index.month)]).sum() / (param['darea'].values[0][0] / 10000)
