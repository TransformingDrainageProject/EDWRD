...
def psthrvst_pstfrz_evap(row, param, data, data_dic, irr_init, zedepl_init):
    ...
    if zedepl_init > param['rew'].values[0][0]:
        data.at[row.Index, 'kr'] = max((param['nongr_tew'].values[0][0] - zedepl_init) / (
            param['nongr_tew'].values[0][0] - param['rew'].values[0][0]), 0)
    else:
        data.at[row.Index, 'kr'] = 1

    # CALCULATE THE EVAPORATION COEFFICIENT (KE, per FAO-56)
    data.at[row.Index, 'ke'] = min(data.at[row.Index, 'kr'] * (data.at[row.Index, 'kcb_max'] -
                                                               data.at[row.Index, 'kcb']), data.at[row.Index, 'few'] * data.at[row.Index, 'kcb_max'])

    # CALCULATE THE EVAPORATION AND TRANSPIRATION COMPONENTS OF ET
    data.at[row.Index, 'evap'] = data.at[row.Index, 'ke'] * \
        data.at[row.Index, 'eto']
    data.at[row.Index, 'trans'] = data.at[row.Index,
                                          'kcb'] * data.at[row.Index, 'eto']

    # CONDUCT THE WATER BALANCE IN THE EVAPORATION LAYER
    data.at[row.Index, 'zeperc'] = max((data.at[row.Index, 'prcp'] - data.at[row.Index, 'ro']) + (
        irr_init / data.at[row.Index, 'fw']) - zedepl_init, 0)
    data.at[row.Index, 'zedepl'] = min(max(zedepl_init - (data.at[row.Index, 'prcp'] - data.at[row.Index, 'ro']) - (irr_init / data.at[row.Index, 'fw']) + (
        data.at[row.Index, 'evap'] / data.at[row.Index, 'few']) + data.at[row.Index, 'zeperc'], 0), param['nongr_tew'].values[0][0])

    # POPULATE THE CROP COEFFICIENT AND CROP ET
    data.at[row.Index, 'kc'] = data.at[row.Index,
                                       'kcb'] + data.at[row.Index, 'ke']
    data.at[row.Index, 'etc'] = data.at[row.Index, 'kc'] * \
        data.at[row.Index, 'eto']
...
def winter_evap(row, param, data, data_dic, irr_init, zedepl_init):
    ...
    data.at[row.Index, 'zeperc'] = max((data.at[row.Index, 'prcp'] - data.at[row.Index, 'ro']) + (
        irr_init / data.at[row.Index, 'fw']) - zedepl_init, 0)
    data.at[row.Index, 'zedepl'] = min(max(zedepl_init - (data.at[row.Index, 'prcp'] - data.at[row.Index, 'ro']) - (irr_init / data.at[row.Index, 'fw']) + (
        data.at[row.Index, 'evap'] / data.at[row.Index, 'few']) + data.at[row.Index, 'zeperc'], 0), param['tew'].values[0][0])
...
def evap(row, param, data, data_dic, irr_init, zedepl_init):
    ...
    # CONDITION: IF DEPLETION IN THE EVAPORATION LAYER > REW THEN A EVAPORATION REDUCTION COEFFICIENT (KR) IS CALCULATED
    if zedepl_init > param['rew'].values[0][0]:
        data.at[row.Index, 'kr'] = max((param['tew'].values[0][0] - zedepl_init) / (
            param['tew'].values[0][0] - param['rew'].values[0][0]), 0)
    else:
        data.at[row.Index, 'kr'] = 1
