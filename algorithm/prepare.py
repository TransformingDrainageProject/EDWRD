...
def edwrd_input(infile,pfile):
    ...
    if param['irrdep_min'].empty:
        param['irrdep_min'] = pd.DataFrame(np.array([0]), columns = param['irrdep_min'].columns)
    ...
    r_vols = [(param['rvol'].at[0,'rvol']*1.5), (param['rvol'].at[0,'rvol']*2.0), (param['rvol'].at[0,'rvol']*0.5), (param['rvol'].at[0,'rvol']*0.1)]

    for i in r_vols:
        param['rvol'] = param['rvol'].append({'rvol':i}, ignore_index=True)

    param['rvol'].sort_values(by=['rvol'], inplace=True) #sort all reservoir volumes in ascending order
    param['rvol'] = param['rvol'].reset_index(drop=True) #reset index

    param['rarea'] = param['rvol'] / param['rdep'].at[0,'rdep'] #recalculate reservoir areas, including for smaller and larger reservoir volumes
    ...
    param['rarea'].at[len(param['rarea']),'rarea'] = param['rarea'].at[0,'rarea']
